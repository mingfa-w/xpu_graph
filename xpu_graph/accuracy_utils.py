...
def assert_close(gold: torch.Tensor, act: torch.Tensor, eval_type: str = "DEFAULT"):
    gold = gold.cpu()
    act = act.cpu()
    if act.dtype in [torch.float16, torch.float32, torch.bfloat16]:
    ...
    eps = eval_standard[act.dtype]["eps"]
    rtol = eval_standard[act.dtype]["rtol"]
    atol = eval_standard[act.dtype]["eps_atol"]
    if eval_type == "DEFAULT":
        ...
        assert (
            torch.sum(not_close).item() < not_close.numel() * eps
        ), "actual tensor are not close enough with golden tensor,\
...
def benchmark_compare_close(gold: torch.Tensor, act: torch.Tensor, std: torch.tensor):
    ...
    assert (
        act_small_error_ratio <= 2 * std_small_error_ratio
    ), "act_small_error_ratio > std_small_error_ratio 's 2 times"
