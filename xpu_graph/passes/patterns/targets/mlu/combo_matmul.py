...
from .triton_kernel.fused_grouped_gemm import fused_grouped_gemm
...
class FusedCombineBmm(nn.Module):
    ...
    def forward_mm(self, input_list, weight_list, bias_list, act: str):
        output = fused_grouped_gemm(
            input_list, weight_list, c=[], bias=[], alpha=[], beta=[], trans_a=False, trans_b=False
        )
        return output
