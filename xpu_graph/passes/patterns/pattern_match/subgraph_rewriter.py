# NOTE(liuyuan): COPY FROM TORCH2.5.1
import copy
...
@compatibility(is_backward_compatible=True)
def replace_pattern(
    gm: GraphModule, pattern: Union[Callable, GraphModule], replacement: Union[Callable, GraphModule]
) -> List[Match]:
    """
...
def _replace_pattern(
    ...
    _matches = [
        m for m in _matches if all(match_filter(m, original_graph, pattern_graph) for match_filter in match_filters)
    ]
    ...
    for match in _matches:
        ...
        match_and_replacements.append(
            ReplacedPatterns(anchor=match.anchors[0], nodes_map=match.nodes_map, replacements=replacement_nodes)
        )
