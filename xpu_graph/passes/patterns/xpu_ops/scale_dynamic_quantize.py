...
class ScaleDynamicQuantize(AutoMatchPattern):
    def rewriter(self, gm: fx.GraphModule, rule_name: str, node_map: dict):
        ...
        with gm.graph.inserting_before(scale_node):
            ...
            if scale_value_node.meta['tensor_meta'].dtype != torch.float:
                scale_value_node = gm.graph.call_function(torch.ops.aten._to_copy.default, args=(scale_value_node,), kwargs={'dtype': torch.float})
            if scale_node.target == torch.ops.aten.div.Tensor:
                scale_value_node = gm.graph.call_function(torch.ops.aten.reciprocal.default, args=(scale_value_node,))

            scale_dynamic_quantize = gm.graph.call_function(torch.ops.xpu_ops.scale_dynamic_quantize.default, args=(scale_node.args[0], scale_value_node))
