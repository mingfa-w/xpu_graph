...
class FlashAttention(Pattern):
    def process(self, gm: fx.GraphModule):
        candidates = [
            ...
            and node.target == torch.ops.npu.npu_prompt_flash_attention.default
        ]
        ...
        for fa_node in candidates:
            if "pse_shift" in fa_node.kwargs:
                continue
            if (
                "input_layout" not in fa_node.kwargs
                or fa_node.kwargs["input_layout"] != "BNSD"
            ):
                continue
            ...
            with maybe_disable_fake_tensor_mode():
                mask_constant = torch.tril(
                    torch.ones(1024, 1024, dtype=torch.uint8).npu()
                )
                mask_name = get_constant_manager(gm).register_constant(
                    mask_constant, "casul_mask"
                )

            with gm.graph.inserting_before(fa_node):
                mask = gm.graph.get_attr(mask_name)
