...
def fn0(x1, x2, x3, x4, x5, x6, x7, x8):
    a = x1 * x2
    sum_a = a.sum(dim=1)
    b = x3 * x4
    sum_b = b.sum(dim=1)
...
def mul_sum_cat_test(xpu_graph_backend, func):
    ...
    e = torch.randn(batch, 128, 128, dtype=dtype, device="mlu")
    f = torch.randn(batch, 128, 128, dtype=dtype, device="mlu")
    g = torch.randn(batch, 128, 128, dtype=dtype, device="mlu")
    h = torch.randn(batch, 128, 128, dtype=dtype, device="mlu")
