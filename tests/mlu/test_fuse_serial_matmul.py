...
def fn0(ffn_input, ffn_weight1, ffn_weight2, ffn_weight3, 
        bias1, bias2, bias3, act1, act2, act3):
...
def serial_mm_test(xpu_graph_backend, func):
    with torch.no_grad():
        ...
        weight3 = torch.randn(
            weight_size2, weight_size3, dtype=dtype, device=device
        )
        ...
        assertTensorsEqual(
            res1.cpu().float(), res.cpu().float(), 0.003, use_MSE=True, use_RAE=True
        )
